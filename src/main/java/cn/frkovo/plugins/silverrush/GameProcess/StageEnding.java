...
public class StageEnding {
    ...
    public static void RunEnding(){
        ...
        for(int i = 0; i < info.rank.size(); i++){
            ...
            for(String s : SilverRush.cfg.getStringList("rw-cmd")){
                Bukkit.dispatchCommand(Bukkit.getConsoleSender(), PlaceholderAPI.setPlaceholders(info.rank.get(i).getPlayer(),s.replace("{rank}",i + 1 + "")));
            }
        }
        Collections.reverse(info.rank);
